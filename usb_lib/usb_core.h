...
#ifdef SEPARATE_USB_BUF // use separate buffers for any USB interface
typedef struct USB_BUFF{
	uint32_t Rx_ptr_in;
	uint32_t Rx_ptr_out;
	uint32_t Rx_length;
	uint8_t  Tx_State;
	__ALIGN_BEGIN uint8_t USB_Rx_Buf[CDC_DATA_MAX_PACKET_SIZE] __ALIGN_END ;
	__ALIGN_BEGIN uint8_t APP_Rx_Buf[APP_RX_DATA_SIZE] __ALIGN_END ;
	__ALIGN_BEGIN uint8_t USB_Cmd_Buf[CDC_CMD_PACKET_SZE] __ALIGN_END ;
} USB_buffer;
#endif
...
{
  uint8_t  (*Init)         (USB_OTG_CORE_HANDLE *pdev , uint8_t cfgidx);
  uint8_t  (*DeInit)       (USB_OTG_CORE_HANDLE *pdev , uint8_t cfgidx);
 /* Control Endpoints*/
  uint8_t  (*Setup)        (USB_OTG_CORE_HANDLE *pdev , USB_SETUP_REQ  *req);
  uint8_t  (*EP0_TxSent)   (USB_OTG_CORE_HANDLE *pdev );
  uint8_t  (*EP0_RxReady)  (USB_OTG_CORE_HANDLE *pdev );
  /* Class Specific Endpoints*/
  uint8_t  (*DataIn)       (USB_OTG_CORE_HANDLE *pdev , uint8_t epnum);
  uint8_t  (*DataOut)      (USB_OTG_CORE_HANDLE *pdev , uint8_t epnum);
...
#else
#define USB_Rx_Buffer	pdev->buf.USB_Rx_Buf
#define APP_Rx_Buffer	pdev->buf.APP_Rx_Buf
#define CmdBuff			pdev->buf.USB_Cmd_Buf
#define APP_Rx_ptr_in	pdev->buf.Rx_ptr_in
#define APP_Rx_ptr_out	pdev->buf.Rx_ptr_out
#define APP_Rx_length	pdev->buf.Rx_length
#define USB_Tx_State	pdev->buf.Tx_State
#endif
